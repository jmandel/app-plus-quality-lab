...
function benchBadge(bench: Bench): { label: string; why: string } {
  ...
  if (bench.kind === "flat percentage") return { label: "FLAT · clinical guardrail", why: "Flat by rule, not data — 42 CFR 414.1380(b)(1)(ii)(C): for measures where chasing top performance risks inappropriate treatment (over-medicating toward hypotension/hypoglycemia), CMS sets flat bands \"for all collection types where the top decile is higher than 90 percent.\" Observed: flat under MIPS CQM and Part B Claims, graded under eCQM, stably since at least 2023 — consistent with a per-collection-type test, though flattening erases the underlying distribution, so the trigger itself isn't visible in the published files." };
