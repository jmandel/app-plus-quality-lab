...
function benchBadge(bench: Bench): { label: string; why: string } {
  ...
  if (bench.kind === "flat percentage") return { label: "FLAT · clinical guardrail", why: "Flat by rule, not data — 42 CFR 414.1380(b)(1)(ii)(C). Two conditions: CMS flags the measure as one where chasing top performance risks inappropriate treatment (over-medicating toward hypotension/hypoglycemia), AND it applies flat bands only to collection types whose top decile exceeds 90%. Registry reporters cross that line; eCQM distributions don't (the capture tax holds them down), which is why the same measure keeps a graded curve under eCQM." };
