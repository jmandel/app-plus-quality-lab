...
function benchBadge(bench: Bench): { label: string; why: string } {
  ...
  if (bench.kind === "flat percentage") return { label: "FLAT · clinical guardrail", why: "Flat by rule, not data — 42 CFR 414.1380(b)(1)(ii)(C), for measures where chasing the top decile risks inappropriate treatment (over-medicating toward hypotension/hypoglycemia). CMS's benchmark fact sheet states the test: flat benchmarks apply \"to collection types where the top decile for a historical benchmark is higher than 90% (or less than 10% for inverse measures)\" — so it is decided per collection type, per year. Confirmed in the last pre-flat data (PY2019): measure 236's top decile was 100% under MIPS CQM and 94.9% under claims — both flattened — while eCQM's was 82.2% and kept its graded curve. The structured-capture gap is what holds electronic reporters below the line." };
