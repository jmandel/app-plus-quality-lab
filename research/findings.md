...
| **CORRECTION 2026-08-08** | This finding's empirical support was **wrong** and is withdrawn. It cited the PY2024 PUF's `Recvd40p` flag (35 ACOs at 77.05) as proof the floor is "a real applied mechanism." Follow-up forensics (`answers-q1-empirical.md`) show `Recvd40p` is the **extreme-and-uncontrollable-circumstances** floor at § 425.512(c)(3)(iii) — all 35 are `DisAffQual=1`, replicated across four years with zero counterexamples, and the PUF has no field for the (a)(7) floor at all. The **(a)(7)(ii)(B) benchmark prong has never been observed to fire**, and PY2024 Medicare CQM (a new collection type with no possible pre-year benchmark) shows CMS instead built performance-period benchmarks and scored normally — 14 of 26 reporting ACOs scored below the floor value unfloored. The recommendation not to implement the floor stands, and is now better supported. |
